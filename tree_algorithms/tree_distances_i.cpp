...
void dfs(int src, vector<VI>& adj, VI& curvis, VI& vis, int dist) {
  if(curvis[src]!=-1)
    return;

  curvis[src]=dist;
  vis[src]=max(vis[src], curvis[src]);

  for(auto it:adj[src])
    dfs(it, adj, curvis, vis, dist+1);
}
...
void solve(int tc) {
  ...
  dfs(farthest, adj, curvis, vis, 0);

  curvis=VI(n+1, -1);
  dfs(farthest2, adj, curvis, vis, 0);
